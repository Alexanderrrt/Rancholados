...
export function validateCart(value: unknown): ValidatedLine[] | null {
  ...
  for (const raw of value) {
    ...
    if (!itemId || !Number.isInteger(quantity) || quantity < 1 || quantity > MAX_QUANTITY) {
      return null;
    }
...
export function validateAddress(value: unknown): CheckoutAddress | null {
  if (typeof value !== "object" || value === null) return null;
  const raw = value as Record<string, unknown>;
  const address: CheckoutAddress = {
    street1: cleanString(raw.street1, 100),
    street2: cleanString(raw.street2, 100),
    city: cleanString(raw.city, 60),
    state: cleanString(raw.state, 2).toUpperCase(),
...
function signingSecret(): string {
  ...
  if (!secret && checkoutMode().demo) {
    return "rancholados-local-demo-signing-secret-only";
  }
...
export function verifyCheckoutToken(token: unknown): CheckoutTokenPayload | null {
  ...
  try {
    ...
    const address = validateAddress(parsed.address);
    const customer = validateCustomer(parsed.customer);
    if (!lines || !address || !customer || typeof parsed.quoteId !== "string") return null;
    const amounts = calculateAmounts(lines, Number(parsed.deliveryFeeCents));
    if (
      !Number.isSafeInteger(parsed.deliveryFeeCents) ||
      parsed.deliveryFeeCents < 0 ||
      amounts.subtotalCents !== parsed.subtotalCents ||
      amounts.taxCents !== parsed.taxCents ||
      amounts.totalCents !== parsed.totalCents
    ) return null;
    return { ...parsed, address, customer, lines: lines.map(({ itemId, quantity }) => ({ itemId, quantity })) };
  } catch {
    return null;
  }
}

export function addressToUber(address: CheckoutAddress): string {
  return JSON.stringify({
    street_address: [address.street1, address.street2],
    city: address.city,
    state: address.state,
